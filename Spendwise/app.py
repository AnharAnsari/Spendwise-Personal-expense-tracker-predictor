...
def add_expense():
    ...
    # Check if amount exceeds 1k limit (can be changed)
    if amount > 1000:
...
def predict():
    ...
    for category in df['category'].unique():
        ...
        future_amounts = model.predict(future_days_poly)
        future_amounts = np.clip(future_amounts, 0, 1000)  # Clip between 0 and 1000
